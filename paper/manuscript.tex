...
\abstract{Automatically matching educational content to curriculum topics is crucial for delivering learning materials to underserved populations, yet most existing benchmarks rely on proprietary data and cover narrow language distributions. We present Wikibooks-ML, an open multilingual benchmark for curriculum--content matching built entirely from Wikibooks, comprising 49,998 curriculum topics and 211,137 content items across six languages (English, German, French, Spanish, Italian, Portuguese). We frame curriculum--content matching as a dual-encoder retrieval problem and apply an InfoNCE contrastive learning framework with multilingual sentence-transformer encoders. To exploit the hierarchical nature of curricula, we analyze H-InfoNCE, a variant that injects curriculum-aware hard negatives derived from sibling nodes in the topic tree starting from the first training epoch. Experiments on a stratified 80/20 holdout (9,999 held-out topics) show that contrastive fine-tuning raises the F2 score from 0.362 (zero-shot) to 0.682 (LaBSE, max len 48), with a longer 96-token context and optimal margin of 0.10 yielding further improvements. A systematic ablation covers two backbones, two sequence lengths, a margin sweep, three negative-mining strategies, a ground-truth-cardinality stratification, and a per-language breakdown. The analysis reveals that curriculum-aware and error-driven mining are complementary, that per-topic performance is governed by ground-truth cardinality rather than training-data volume, and that the standard F2+dynamic-threshold evaluation underestimates neural rankers in the low-similarity-spread regime. The dataset and code are released to support reproducible research on equitable educational recommendation.}
...
Table~\ref{tab:backbone} compares two encoder backbones and two sequence lengths after contrastive fine-tuning. The paraphrase-mpnet encoder reaches 0.670 at a 48-token context and a longer 96-token context adds a further +0.010. LaBSE, a higher-capacity encoder (471M parameters), is more sensitive to the learning rate: at the default learning rate of $2\times10^{-4}$ its training became unstable, with the validation F2 peaking at 0.640 in epoch 5 and then collapsing to near zero. Reducing the learning rate to $1\times10^{-4}$ with a 3-epoch warmup and a $1\times10^{-5}$ end learning rate eliminated the instability and yielded a stable best F2 of 0.682, the highest configuration tested. We note this as a practical finding: high-capacity multilingual encoders require more conservative optimization schedules under contrastive fine-tuning, consistent with the higher gradient variance of large in-batch objectives.

For the remaining analyses (negative-mining ablation, margin sweep, per-language breakdown, embedding geometry) we use the paraphrase-mpnet model at a 48-token context, as its training is stable and it isolates the effect of each design choice; the LaBSE result above establishes the achievable ceiling.
...
These findings come with limitations that we detail in Section~\ref{sec:discussion}: same-language ground truth by construction, a residual channel-leakage risk in the topic-level split, a highly skewed ground-truth cardinality, and single-run results. The benchmark could also be extended to additional, lower-resource languages and to non-textual content modalities. We release the dataset construction pipeline, training code, and full experimental configuration to support reproducible research on equitable, multilingual educational recommendation.
