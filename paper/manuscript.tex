...
Without fine-tuning, the dual-encoder models already rank correct content highly but fail to clear the dynamic-threshold criterion, producing a regime in which a simple lexical baseline outscores them on F2 despite lower ranking quality. Table~\ref{tab:zeroshot} reports the details: the neural encoders achieve MRR of 0.662--0.669, whereas the non-neural title-overlap baseline cannot be ranked meaningfully; yet on the threshold-sensitive F2 the lexical baseline leads (0.539 vs.\ 0.317--0.362). This threshold-induced discrepancy motivates the evaluation analysis in Section~\ref{sec:discussion}.
...
\caption{Validation F2 during contrastive fine-tuning. Curriculum-aware negatives (both H-InfoNCE variants) accelerate convergence during the warm-up phase (epochs 1--4), whereas dynamic mining drives the epoch-5 jump; the two mechanisms converge at epoch 10.\label{fig:curve}}
\end{figure}
...
\caption{Effect of the margin threshold on F2, precision, and recall. F2 peaks at margin 0.10 (0.675), exceeding the default 0.16; precision falls monotonically while recall rises, confirming a precision--recall trade-off.\label{fig:margin}}
\end{figure}
...
Performance varies sharply across languages in a pattern that contradicts the usual data-volume intuition: German achieves the highest F2 (0.914) while English, the largest partition, achieves the lowest (0.340) (Figure~\ref{fig:radar}). Manual inspection attributes the English deficit to ``list-style'' topics (e.g., keyword references, award lists) with many ground-truth items; we quantify this mechanism in the next subsection. Consistent with this heterogeneity, the per-topic F2 distribution is strongly bimodal (Figure~\ref{fig:f2dist}): 50.3\% of topics achieve F2$\geq$0.9 while 12.3\% fail completely (F2=0).
